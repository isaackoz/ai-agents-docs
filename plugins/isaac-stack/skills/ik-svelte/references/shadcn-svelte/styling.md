...
Use the project's shadcn-svelte global CSS file from `components.json` for theme variables and custom colors.
...
If you need a success/positive color that does not exist as a semantic token, use a Badge variant or ask the user about adding a custom CSS variable to the theme.
...
3. **CSS variables** — define custom colors in the global CSS file from `components.json`.
